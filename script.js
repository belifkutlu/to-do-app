...
const getDonesFormStorage = () => {
  const storage = JSON.parse(localStorage.getItem("dones"));
  return storage ? storage : [];
};

const todos = getTodosFormStorage();
const dones = getDonesFormStorage();
...
const listFilter = () => {
  ...
  array.forEach((item) => {
    switch (filter) {
      ...
      case "2":
        todoFilter.className = "";
        todoFilter.classList.remove("fa-square");
